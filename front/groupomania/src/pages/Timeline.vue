    ...
    <div class="wrapper">
      <aside class="people">
        <div class="people-wrapper">
          <h2>People</h2>
          <div
            class="people__detail-wrapper"
            v-for="profile in profileArray"
            :profile="profile"
            :key="profile.index"
          >
            <router-link
      ...
      <div class="post-wrapper">
        ...
        <div class="top-post">
          <div
            class="top-post__wrapper"
            v-for="post in TopPostArray"
            :post="post"
            :key="post.index"
          >
            <router-link :to="'/post/' + post.post_id">
              <img
                :src="post.media"
                alt="image of top post"
                class="top-post__picture"
              />
      ...
      <div
        class="tint"
        v-if="render('/makeapost')"
        @click="tintReRoute('/timeline')"
      ></div>
      '
      <form @submit.prevent="makePost" v-if="render('/makeapost')">
        <base-module class="make-post">
          <template #title> Make A Post </template>
          <template #form-content>
            <p v-if="!imageSelected" class="validation">no image selected</p>
            ...
            <div class="make-post__btn-wrapper">
              ...
              <div class="make-post__btns-wrapper">
                ...
                <input
                  ...
                  if="file"
                  name="file"
                  class="make-post__btn"
                  @change="onFileSelected"
                />
                <base-button type="submit" class="make-post__btn"
                  >Post</base-button
                >
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    async getTopPost() {
      const data = await this.fetchGet(
        "http://localhost:3000/api/post/mostinteractions"
      );
    ...
    async getPost() {
      ...
      const data = await this.fetchWithBody(
        "http://localhost:3000/api/post/allpost",
        body,
        "POST"
      );
    ...
    async mostLikes() {
      const data = await this.fetchGet(
        "http://localhost:3000/api/post/mostlikes"
      );
    ...
    async mostCommented() {
      const data = await this.fetchGet(
        "http://localhost:3000/api/post/mostcommented"
      );
